...
const CommuteSuggestions: React.FC = () => {
    ...
    const suggestions: CommuteSuggestion[] = [
        ...
        },
        {
            ...
            arrivalTime: '08:42 AM',
            congestion: 'Low',
            weatherImpact: weatherData?.condition?.toLowerCase().includes('rain') ? 'High' : 'Low',
            score: weatherData?.condition?.toLowerCase().includes('rain') ? 55 : 78
        }
    ];
    ...
    const filteredSuggestions = userPreferences.preferredTransportModes?.length > 0
        ? suggestions.filter(
            suggestion => userPreferences.preferredTransportModes.some(
                mode => suggestion.mode.toLowerCase().includes(mode.toLowerCase())
            )
          )
        : suggestions;
